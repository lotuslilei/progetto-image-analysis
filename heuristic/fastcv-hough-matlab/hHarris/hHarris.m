function [ imagePoints badImages ] = hHarris( imagesPath, numImages, flagPlot )
%HHARRIS for each image it tries to extract the four corner points of the
...
for i = 1:numImages
    ...
    msg = ['hHarris: processing image ', num2str(i)];
    disp(msg);
    ...
    try
        ...
        imagePoints(i).pts = [CxMin(1,1) CxMin(1,2); CxMax(1,1) CxMax(1,2); CyMin(1,1) CyMin(1,2); CyMax(1,1) CyMax(1,2)];
        ...
        disp(fprintf('Immagine %d non gestibile...', i));
        imagePoints(i).pts = [];
        imagePoints(i).id = strcat(num2str(i));
        badImages(i) = 1;
    end
...
save(strcat(imagesPath, '/imagePointshHarris'), 'imagePoints');
save(strcat(imagesPath, '/badImageshHarris'), 'badImages');

end

function [ output_args ] = copiaSenzaElemento(array, index, nR)
output_args = zeros(nR-1, 2);

for j=1:index-1
    output_args(j,1) = array(j,1);
    output_args(j,2) = array(j,2);
end

for j=index+1:nR
    output_args(j-1,1) = array(j,1);
    output_args(j-1,2) = array(j,2);
end
end
