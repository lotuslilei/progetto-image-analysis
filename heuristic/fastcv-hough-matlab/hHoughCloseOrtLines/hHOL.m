function [imagePoints badImages] = hHOL( imagesPath, numImages, flagPlot )
%HHOL for each image it tries to extract the four corner points of the
...
for i = 1:numImages
    ...
    msg = ['hHOL: processing image ', num2str(i)];
    disp(msg);
    ...
    imagePoints(i).pts = getPointsOrt(candLines, thetaToll);
    imagePoints(i).id = strcat(num2str(i));
...
save(strcat(imagesPath, '/imagePointshHOL'), 'imagePoints');
save(strcat(imagesPath, '/badImageshHOL'), 'badImages');
