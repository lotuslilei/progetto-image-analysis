function [imagePoints badImages] = hProportion( imagesPath, numImages, flagPlot )
%HPROPORTION for each image it tries to extract the four corner points of the
%sheet in the image, according to the Proportion heuristic
%
...
for i = 1:numImages
    ...
    msg = ['hProportion: processing image ', num2str(i)];
    disp(msg);
    ...
    if (exist('lines', 'var')==0)
        ...
        badImages(i) = 1;
        clear lines sLines candLines candLinesProp
        continue;
    end
    ...
    % part of that line (desired value = 12)
    distToll = ceil(0.005*(ri + ci)/2);
    ...
    % Distance tollerance for having 2 close lines (desired value = 12)
    distToll = ceil(0.005*(ri + ci)/2);
    ...
    % It tries to find lines that respects the constraint tollA4, relaxing it by its increment tollIncA4, till it reachs tollMaxA4
    while(tollA4<=tollMaxA4)
        
        for c1 = 1:length(candLines)
            
            from = c1 + 1;
            
            if(from<=length(candLines))
                for c2 = from:length(candLines)
                    
                    candLinesProp(1) = candLines(c1);
                    candLinesProp(2) = candLines(c2);
                    
                    % Extract the corner points
                    imagePoints(i).pts = getPointsOrt(candLinesProp, thetaToll);
                    imagePoints(i).id = strcat(num2str(i));
                    
                    if(insideImage(imagePoints(i).pts, ri, ci) && arePropA4(imagePoints(i).pts, tollA4, ri, ci))
                        inside = true;
                        prop = true;
                        break;
                    end
        ...
        tollA4 = tollA4 + tollIncA4;
        
    end
    
    if(inside == false || prop == false)
        imagePoints(i).pts = [];
        imagePoints(i).id = strcat(num2str(i));
        badImages(i) = 1;
        clear lines sLines candLines candLinesProp
...
save(strcat(imagesPath, '/imagePointshProportion'), 'imagePoints');
save(strcat(imagesPath, '/badImageshProportion'), 'badImages');
