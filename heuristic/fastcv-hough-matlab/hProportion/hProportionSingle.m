function [ imagePoints ] = hProportionSingle( image, flagPlot )
%HPROPORTIONSINGLE given the image it tries to extract the four corner points of the
    ...
    for c1 = 1:length(candLines)
        ...
        if(from<=length(candLines))
            for c2 = from:length(candLines)
                ...
                imagePoints = getPointsOrt(candLinesProp, thetaToll);
                
                if(insideImage(imagePoints, ri, ci) && arePropA4(imagePoints, tollA4, ri, ci))
                    inside = true;
                    prop = true;
                    break;
                end
