...
void updateFilterCorr( uint8_t *pSrc, uint32_t w, uint32_t h, uint8_t *pDst )
{
	   int i;
	   int j;
	   const int max_knl_size = 17; // Max Kernel Size used in this test

	   //Declaration of kernels
	   static short FASTCV_ALIGN128( knl17[max_knl_size] ) = {
	      148, 327, 646, 1149, 1841, 2656, 3451, 4038, 4255, 4038, 3451, 2656, 1841,
	      1149, 646, 327, 148 };

	   signed char FASTCV_ALIGN128 ( mask3x3[9] ) = {32, 1, -32, 64, 1, -64, 32, 1, -32 };

	   // 3-sigma kernels (Q15 normalization)
	   static short FASTCV_ALIGN128( knl9[9] ) = {
	      52, 535, 2820, 7646, 10661, 7646, 2820, 535, 52 };

	   static short FASTCV_ALIGN128( knl11[12] ) = {      // N-TAP 11 with 0 pad
	      44, 297, 1285, 3662, 6864, 8463, 6864, 3662, 1285, 297, 44, 0 };

	   static short FASTCV_ALIGN128( knl13[13] ) = {
	      58, 248, 814, 2049, 3964, 5890, 6721, 5890, 3964, 2049, 814, 248, 58 };

	   static short FASTCV_ALIGN128( knl15[16] ) = {      // N-TAP 15 with 0 pad
	      90, 268, 669, 1413, 2528, 3830, 4915, 5341, 4915, 3830, 2528, 1413, 669,
	      268, 90, 0 };
	   ...
	   // Perform FastCV Function processing
		switch( filtercorrState.kernelType )
		{
				...
				for(i = 0; i < h; i++)
				   {
					   for(j = 0; j < w; j++)
						 {
							filtercorrState.filteredInputBufs16[i*h+j] = (int16_t)pSrc[i*h+j];
						 }
				   }
				fcvFilterCorrSep9x9s16	( knl9, filtercorrState.filteredInputBufs16, w, h,  filtercorrState.scratchBuffer, filtercorrState.filteredOutputBufs16);
				for(i = 0; i < h; i++)
				   {
						for(j = 0; j < w; j++)
						{
							pDst[i*h+j] = (uint8_t) ((filtercorrState.filteredOutputBufs16[i*h+j]/2) + 128);
						}
					}
				break;
			...
			case ELEVENxELEVEN:
				...
				fcvFilterCorrSep11x11s16	( knl11, filtercorrState.filteredInputBufs16, w, h,  filtercorrState.scratchBuffer, filtercorrState.filteredOutputBufs16);
				for(i = 0; i < h; i++)
				   {
						for(j = 0; j < w; j++)
						{
			...
			case FIFTEENxFIFTEEN:
				...
				fcvFilterCorrSep15x15s16	( knl15, filtercorrState.filteredInputBufs16, w, h,  filtercorrState.scratchBuffer, filtercorrState.filteredOutputBufs16);
				for(i = 0; i < h; i++)
				   {
						for(j = 0; j < w; j++)
						{
			...
			case SEVENTEENxSEVENTEEN:
				...
				fcvFilterCorrSep17x17s16	( knl17, filtercorrState.filteredInputBufs16, w, h,  filtercorrState.scratchBuffer, filtercorrState.filteredOutputBufs16);
				for(i = 0; i < h; i++)
				   {
						for(j = 0; j < w; j++)
						{
